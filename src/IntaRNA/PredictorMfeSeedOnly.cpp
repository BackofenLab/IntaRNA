...
predict( const IndexRange & r1, const IndexRange & r2 )
{
...
#endif
	{ VLOG(2) <<"predicting seed interactions only..."; }
	// measure timing
	...
	if (!(r1.isAscending() && r2.isAscending()) )
		throw std::runtime_error("PredictorMfeSeedOnly::predict("+toString(r1)+","+toString(r2)+") is not sane");
#endif
...
traceBack( Interaction & interaction )
{
	...
	// sanity checks
	if ( interaction.basePairs.size() != 2 ) {
		throw std::runtime_error("PredictorMfeSeedOnly::traceBack() : given interaction does not contain boundaries only");
	}
#endif
	...
	if ( ! interaction.isValid() ) {
		throw std::runtime_error("PredictorMfeSeedOnly::traceBack() : given interaction not valid");
	}
#endif
		...
		|| (j2 != i2+seedHandler.getSeedLength2(i1,i2)-1) )
	{
		...
		throw std::runtime_error("PredictorMfeSeedOnly::traceBack() : given boundaries "+toString(interaction)+" are not only containing a seed");
	}
#endif
...
{
	INTARNA_NOT_IMPLEMENTED("PredictorMfeSeedOnly::getNextBest() coming soon...");

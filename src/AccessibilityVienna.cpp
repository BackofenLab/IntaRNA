...
AccessibilityVienna::AccessibilityVienna(
	...
	partFoldParams(NULL)
{
	...
	const double pfScale = getPfScale( seq, modelDetails, T );

	// Vienna RNA : get final partition function folding parameters
	partFoldParams = get_boltzmann_factors((double)T, 1.0, modelDetails, pfScale);


	// compute free energy of whole structure ensemble
	E_type E_all = calc_ensemble_free_energy(-1, -1, T);
	...
	for(int i=0; i<seq_len; i++)
	{
		...
		for(int j=i; j<std::min(seq_len,i+(int)maxLength-1); j++)
		{
			...
			if (regionUnconstrained) {
				...
				edValues (i,j) = (calc_ensemble_free_energy(i,j,T) - E_all);
			} else {
...
double
AccessibilityVienna::
getPfScale( const RnaSequence & seq, const model_detailsT & modelDetails, const T_type T )
{
	...
	paramT * foldParams = get_scaled_parameters((double)T, modelDetails);
	...
	const double RT = GASCONST*((double)T+K0)/1000.0;

...
//	LOG(DEBUG) <<"r2 : "<<rAcc2.getSequence();
	...
	SeedConstraint sConstr(3,0,0,0,0,10,0,IndexRangeList(),IndexRangeList(),"", false);
...
//					LOG(DEBUG) <<"seed at "<<i1<<"-"<<i2;
	...
	SECTION( "no GU bp" ) {
		...
		{
			// should find no seed
			SeedConstraint sConstr(3,0,0,0,0,10,0,IndexRangeList(),IndexRangeList(),"", true);
			SeedHandlerNoBulge sh(energy,sConstr);
			REQUIRE( sh.fillSeed(0,energy.size1()-1,0,energy.size2()-1) == 0 );
		}
	...
	SECTION( "maxE" ) {
...

		{
			...
			SeedConstraint sConstr(3,0,0,0,Ekcal_2_E(-1),10,0,IndexRangeList(),IndexRangeList(),"", false);
			SeedHandlerNoBulge sh(energy,sConstr);
			REQUIRE( sh.fillSeed(0,energy.size1()-1,0,energy.size2()-1) > 0 );
		}
		{
			// should find some seed
			SeedConstraint sConstr(3,0,0,0,Ekcal_2_E(-2),10,0,IndexRangeList(),IndexRangeList(),"", false);
			SeedHandlerNoBulge sh(energy,sConstr);
			REQUIRE( sh.fillSeed(0,energy.size1()-1,0,energy.size2()-1) > 0 );
		}
		{
			// should find no seed
			SeedConstraint sConstr(3,0,0,0,Ekcal_2_E(-3),10,0,IndexRangeList(),IndexRangeList(),"", false);
			SeedHandlerNoBulge sh(energy,sConstr);
			REQUIRE( sh.fillSeed(0,energy.size1()-1,0,energy.size2()-1) == 0 );
		}
	...
	SECTION( "maxEhybrid" ) {
		...
		{
			// should find some seeds
			SeedConstraint sConstr(3,0,0,0,0,10,Ekcal_2_E(-2),IndexRangeList(),IndexRangeList(),"", false);
			SeedHandlerNoBulge sh(energy,sConstr);
			REQUIRE( sh.fillSeed(0,energy.size1()-1,0,energy.size2()-1) > 0 );
		}
		{
			// should find no seed
			SeedConstraint sConstr(3,0,0,0,0,10,Ekcal_2_E(-3),IndexRangeList(),IndexRangeList(),"", false);
			SeedHandlerNoBulge sh(energy,sConstr);
			REQUIRE( sh.fillSeed(0,energy.size1()-1,0,energy.size2()-1) == 0 );
		}

...
TEST_CASE( "InteractionEnergyBasePair - data access", "[InteractionEnergyBasePair]" ) {
...
TEST_CASE( "InteractionEnergyBasePair - energy computation", "[InteractionEnergyBasePair]" ) {

	RnaSequence rna("test","AGCU");

	AccessibilityDisabled acc(rna);
	ReverseAccessibility rAcc(acc);

	size_t maxLoop1 = 1, maxLoop2 = 1;
	InteractionEnergyBasePair energy( acc, rAcc, maxLoop1, maxLoop2);
